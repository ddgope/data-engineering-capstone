...
import sql_queries
from helpers.helper import get_extra_from_conn
...
PARAMS = {
          ...
          'redshift_user': sql_queries.AirflowConnIds.REDSHIFT,
...
# Drop & Create Tables
for table in sql_queries.General.TABLES:
    logging.info(f"Drop & Create {table}")
    PARAMS['table'] = table
    PARAMS['s3_uri'] = ("s3://sql_queries{base_bucket}/{table}.csv".format(**PARAMS))
    drop_stmt = sql_queries.SQLQueries.DROP_TABLE.format(**PARAMS)
    create_stmt = sql_queries.SQLQueries.CREATE[table]
    grant_usage_stmt = sql_queries.SQLQueries.GRANT_USAGE.format(**PARAMS)
    grant_select_stmt = sql_queries.SQLQueries.GRANT_SELECT.format(**PARAMS)
    ...
    if table in sql_queries.General.CSV_TABLES:
